...
Deck rules enforced at load: exactly 60 cards, max 4 copies per name (basic Energy exempt).
...
- `weakness` doubles incoming attack damage; `resistance` subtracts 30. Both optional.
...
| op | fields | meaning |
|----|--------|---------|
| `damage` | `amount`, `target`, `applyWR?` | Attack damage; weakness/resistance applied when hitting the Defending Pokemon (set `applyWR` to force it on/off, e.g. for bench snipes) |
| `damageScaled` | `base`, `amount`, `per` | Damage to the Defending Pokemon: `base` + `amount` × count of `attackerEnergy` / `defenderEnergy` / `defenderDamageCounters` / `yourBench` / `oppBench` |
...
| `discardSelfEnergy` | `count`, `energyType?` | Discard energy from the attacking Pokemon |
...
| `damagePerHeads` | `flips`, `amount`, `target` | Flip N coins, `amount` damage per heads |
| `searchDeck` | `filter`, `count` | Pick matching cards from deck into hand, then shuffle |
| `switchSelf` | — | Switch your Active with a chosen Benched Pokemon |
...
| `attachEnergyFromHand` | `energyType`, `target` | Extra attachment from hand (Water Call) |
